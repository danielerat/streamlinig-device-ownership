...
class Device(models.Model):
    ...
    serial_number = models.CharField(
        verbose_name="Serial Number", max_length=255, unique=True)
    mac_address = models.CharField(
        verbose_name="Mac Address", max_length=100, unique=True)
    imei = models.CharField(verbose_name="IMEI Address",
                            max_length=100, unique=True)
    price = models.DecimalField(
        max_digits=9, decimal_places=2, validators=[MinValueValidator(1)])
    category = models.CharField(
        choices=DEVICE_CATEGORY_CHOICES, blank=False, default="phone", max_length=50)
    desc = models.TextField(verbose_name="Device Description")
    quality = models.PositiveIntegerField(choices=DEVICE_QUALITY_CHOICES, blank=False, null=False, default=2, validators=[
...
class Transfer(models.Model):
    ...
    transferee = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name="transferees")
    transfer_status = models.CharField(
        choices=TRANSFER_STATUS, max_length=1, default="P", null=False, blank=False)
...
class PendingTransfer(models.Model):
    ...
    device = models.ForeignKey(
        Device, on_delete=models.CASCADE, null=False, blank=False)
    transferor = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name="pending_transferors")
    transferee = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name="pending_transferees")
    transfer_status = models.CharField(
        choices=TRANSFER_STATUS, max_length=1, default="P", null=False, blank=False)
    last_confirm = models.DateTimeField(auto_now=True)
    created = models.DateTimeField(auto_now_add=True)
